...
-- -----------------------------------------------------
-- Table `ecolonia`.`plan_trabajo`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ecolonia`.`plan_trabajo` ;

CREATE TABLE IF NOT EXISTS `ecolonia`.`plan_trabajo` (
...
CREATE TABLE IF NOT EXISTS `ecolonia`.`comitedebarrio` (
  ...
  INDEX `fk_comitedebarrio_plan_trabajo1_idx` (`plan_trabajo_Id` ASC),
  CONSTRAINT `FK_ComiteDeBarrio_Colonia`
    FOREIGN KEY (`Colonia`)
    REFERENCES `ecolonia`.`colonia` (`Id`)
    ON DELETE NO ACTION
    ON UPDATE CASCADE,
  CONSTRAINT `fk_comitedebarrio_plan_trabajo1`
    FOREIGN KEY (`plan_trabajo_Id`)
    REFERENCES `ecolonia`.`plan_trabajo` (`Id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
